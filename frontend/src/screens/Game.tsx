...
import ChessLoading from "../components/Loadings/ChessLoading";
...
const Game = () => {
  ...
  const [currentPlayerInCheck, setCurrentPlayerInCheck] =
    useState<Boolean>(false);
  const [opponentInCheck, setOpponentInCheck] = useState<Boolean>(false);
  ...
  const makeMove = (message: {
    ...
    setBoard(chess.board());
    setCurrentUserTurn(chess.turn() === currentUserPieceColor.current);
  };
        ...
        <div className="grid grid-cols-5 w-full gap-4">
          <div className="col-span-4 w-full">
            <ChessBoard
              ...
              chess={chess}
              setBoard={setBoard}
              currentUserPieceColor={currentUserPieceColor}
              gameStatus={gameStatus}
              setCurrentUserTurn={setCurrentUserTurn}
              setGameStatus={setGameStatus}
              currentPlayerInCheck={currentPlayerInCheck}
              setCurrentPlayerInCheck={setCurrentPlayerInCheck}
...
const GameOverPanel = ({ winner }: { winner: "black" | "white" | null }) => {
  return <div className="text-2xl">Winner is {winner}</div>;
};
